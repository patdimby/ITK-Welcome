...
GridMvc = (function ($) {
    ...
    gridMvc.prototype.init = function () {
        ...
        this.addFilterWidget(new MultiCheckboxListFilterWidget());
    ...
    */
    gridMvc.prototype.filterMenuHtml = function () {
        return '<div class="filter-content dropdown dropdown-menu grid-dropdown" style="display: none;">\
    ...
    */
    gridMvc.prototype.getClearFilterButton = function () {
        return '<ul class="menu-list">\
                    <li><button class="btn btn-wme btn-dark w-100 grid-filter-clear" href="javascript:void(0);">' + this.lang.clearFilterLabel + '</button></li>\
                </ul>';
    };
    /***
    ...
    */
    gridMvc.prototype.applyFilterValues = function (initialUrl, columnName, values, skip) {
        ...
        var idGrid = this.jqContainer.attr("id");
        window.location.search = initialUrl + url + "&grid=" + idGrid;
    };
...
GridMvc.lang.en = {
    ...
    selectCategoriesToFilter: "Select the category to filter",
    selectSubCategoriesToFilter: "Select the subcategory to filter",
    selectActionsToFilter: "Select the action to filter",
    selectVersionToFilter: "Select the version to filter",
    ...
    textFilterWidget.prototype.renderWidget = function () {
        var html = '<div class="form-group">\
                    ...
                    <div class="grid-filter-buttons">\
                        <button type="button" class="btn btn-wme w-100 btn-green grid-apply" >' + this.lang.applyFilterButtonText + '</button>\
                    </div>';
...
NumberFilterWidget = (function ($) {
    ...
    numberFilterWidget.prototype.renderWidget = function () {
        var html = '<div class="form-group">\
                    ...
                    <div class="grid-filter-buttons">\
                        <button type="button" class="btn btn-wme btn-green w-100 grid-apply">' + this.lang.applyFilterButtonText + '</button>\
                    </div>';
        this.container.append(html);
    };
        ...
        var html = '<div class="form-group">\
                        ...
                        (this.datePickerIncluded ?
                            ...
                            '<div class="form-group">\
                             ...
                             <div class="grid-filter-buttons">\
                                <input type="button" class="btn btn-wme btn-green w-100 grid-apply" value="' + this.lang.applyFilterButtonText + '" />\
                             </div>');
        this.container.append(html);
...
BooleanFilterWidget = (function ($) {
    ...
    booleanFilterWidget.prototype.renderWidget = function () {
        var html = '<label><b>' + this.lang.selectValue + '</b></label>\
                    <ul class="menu-list">\
...
MultiCheckboxListFilterWidget = (function ($) {
    function multicheckboxListWidget() {
        multicheckboxListWidget.prototype.getAssociatedTypes = function () {
            return ["MulticheckboxListWidget"];
        };
        /***
        * This method invokes when filter widget was shown on the page
        */
        multicheckboxListWidget.prototype.onShow = function () {
            /* Place your on show logic here */
        };

        multicheckboxListWidget.prototype.showClearFilterButton = function () {
            return true;
        };
        /***
        * This method will invoke when user was clicked on filter button.
        ...
        * data - widget data passed from the server
        */
        multicheckboxListWidget.prototype.onRender = function (container, lang, typeName, values, cb, data) {
            //store parameters:
            this.cb = cb;
            this.container = container;
            this.lang = lang;
        ...
        multicheckboxListWidget.prototype.renderWidget = function () {
            var filters = this.value.filterValue.split('|');
            var items = this.items;
            var html = '<div class="form-group">' +
                '<div class="grid-filter-type enums-checklist form-control" style="width:250px;height:250px;overflow-y:auto;">';
            for (var i = 0; i < items.length; i++) {
                //html += '<option ' + (items[i] == this.value.filterValue ? 'selected="selected"' : '') + ' value="' + items[i]['Value'] + '">' + items[i]['Text'] + '</option>';
                html += '<div><input type="checkbox" value="' + items[i]['Value'] + '"' + (filters.indexOf(items[i]['Value']) > -1 ? 'checked="checked"' : '') + '>' + items[i]['Text']+'</div>';
            }
        ...
        * Internal method that register event handlers for 'apply' button.
        */
        multicheckboxListWidget.prototype.registerEvents = function () {
            var $context = this;
            this.container.find('.grid-apply').click(function () {
                var ckvalues = "";
                var controls = $context.container.find('input:checked', 'enums-checklist');
                ...
                var values = [{ filterValue: ckvalues, filterType: 1 /* Equals */ }];
                $context.cb(values);
            });
        };
    }
...
function toogle($this) {
    var self = $this;
    $this.toggleClass("expand collapsed", function () {
        var expanded = self.data("expanded");
        if (expanded) {
            //console.log("expanded", "false");
            self.data("expanded", false);
            self.find("i").removeClass("fa-minus-square");
            self.find("i").addClass("fa-plus-square");
            $("tr[data-parent='" + self.attr('id') + "']").hide();
            return "collapsed";
        }
        else {
            //console.log("expanded", "true");
            self.data("expanded", true);
            self.find("i").removeClass("fa-plus-square");
            self.find("i").addClass("fa-minus-square");
            $("tr[data-parent='" + self.attr('id') + "']").show();
            return "expand";
        }
    });
}
...
(function ($) {
    ...
    $(function () {
        ...
        $(document).on('click', 'tr.category', function (e) {
            ...
            $(this).toggleClass("expand collapsed", function () {
                ...
                if (expanded) {
                    ...
                    self.find("i").removeClass("fa-minus-square");
                    self.find("i").addClass("fa-plus-square");
                    $("tr[data-parent='" + self.attr('id') + "']").each(function (index, obj) {
                        var hassubCategory = $(obj).hasClass("subcategory");
                        expandsubcateg = $(obj).hasClass("expand");
                        if (hassubCategory) {
                            //console.log('hassubCategory=', hassubCategory);
                            //console.log('expandsubcateg=', expandsubcateg);
                            if (expandsubcateg) {
                                $("tr[data-parent='" + $(obj).attr('id') + "']").hide();
                            }
                            $(obj).hide();
                        }
                        else {
                    ...
                    $("tr[data-parent='" + self.attr('id') + "']").each(function (index, obj) {
                        ...
                        if (hassubCategory) {
                            ...
                            //console.log('expandsubcateg=', expandsubcateg);
                            if (expandsubcateg) {
                                //console.log("expanded", $(obj).text());
                                $("tr[data-parent='" + $(obj).attr('id') + "']").show();
                            }
                            $(obj).show();
                        }
                        else {
        ...
        $(document).on('click', 'tr.subcategory', function (e) {
            toogle($(this));
        });
    });
})(window.jQuery);