...
function TypeIntitekUserFilterWidget() {
    ...
    */
    ...
    this.renderWidget = function () {
        ...
        var html = "";
        var template = Handlebars.compile($("#renderwidget-template-type-intitekUser").html());
        html = template({ lang_selectClientToFilter: this.lang.selectValue, lang_applyFilterButtonText: this.lang.applyFilterButtonText, filters: filters });
   ...
   */
    this.registerEvents = function () {
        ...
        var customerList = this.container.find(".typeUserslist");
        ...
        applyBtn.click(function () {
            ...
            var value = "";
            $('.typeUserslist input:checked').each(function (index, element) {
                if (index == 0) {
                    value += element.value;
                }
                else {
...
function selectMetier() {
    console.log('metier');
    $("input[name='Type'][value='MET']").prop("checked", true);
}
function selectStructure() {
    $("input[name='Type'][value='STR']").prop("checked", true);
    console.log('struct');
}