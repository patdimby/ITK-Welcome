...
function CheckboxesFilterWidget() {
    /***
    ...
    */
    this.onRender = function (container, lang, typeName, values, cb, data) {
        ...
        this.loadData(); //load customer's list from the server
    ...
    this.renderWidget = function () {
        var html = "";
        var template = Handlebars.compile($("#renderwidget-template-checkbox").html());
    ...
    */
    this.fillData = function (data) {
        ...
        var dataList = this.container.find(".checkboxlist");

        var checkboxId, checkboxLabel;
        for (var i = 0; i < data.Items.length; i++) {
            //Get checkbox id and label from JSON, otherwise just use the parent object as id and label
            checkboxId = data.Items[i].ID ? data.Items[i].ID : data.Items[i];
            checkboxLabel = data.Items[i].Label ? data.Items[i].Label : data.Items[i];

            dataList.append('<input type = "checkbox"' + (filters.indexOf(checkboxId + "") > -1 ? 'checked="checked"' : '') + ' value="' + checkboxId + '">' + checkboxLabel + '</input><br />');
        }
    };
    /***
    ...
    */
    this.registerEvents = function () {
        ...
        var customerList = this.container.find(".checkboxlist");
        ...
        applyBtn.click(function () {
            ...
            $('.checkboxlist input:checked').each(function (index, element) {
                if (index == 0) {
                    value += element.value;
                }
                else {
