...
function CategoriesFilterWidget() {
    /***
 ...
 this.onRender = function (container, lang, typeName, values, cb, data) {
    ...
    this.loadCategories(); //load customer's list from the server
    ...
    var html = "";
    var template = Handlebars.compile($("#renderwidget-template-categories").html()); 
    if ("Version" === this.columName){
        html = template({ lang_selectClientToFilter: this.lang.selectVersionToFilter, lang_applyFilterButtonText: this.lang.applyFilterButtonText });
    }
    else {
        html = template({ lang_selectClientToFilter: this.lang.selectCategoriesToFilter, lang_applyFilterButtonText: this.lang.applyFilterButtonText });
    }
...
*/
    ...
    */
    this.fillCategories = function (items) {
        var filters = this.value.filterValue.split('|');
        var customerList = this.container.find(".categorieslist");
        for (var i = 0; i < items.length; i++) {
            var isDefaultLangeName = items[i].IsDefaultLangName;
            var name = "<span>" + items[i].Name + "</span>";
            if (isDefaultLangeName)
                name = '<span class="defaultName">' + items[i].Name + '</span>';
            customerList.append('<input type = "checkbox"' + (filters.indexOf(items[i].ID + "") > -1 ? 'checked="checked"' : '') + ' value="' + items[i].ID + '">' + name + '</input><br />');
        }
    };
    this.fillVersions = function (items) {
        var filters = this.value.filterValue.split('|');
        var customerList = this.container.find(".categorieslist");
        for (var i = 0; i < items.length; i++) {
            customerList.append('<input type = "checkbox"' + (filters.indexOf(items[i] + "") > -1 ? 'checked="checked"' : '') + ' value="' + items[i] + '">' + items[i] + '</input><br />');
        }
    };
    /***
    ...
    */
    this.registerEvents = function () {
        ...
        applyBtn.click(function () {
            ...
            $('.categorieslist input:checked').each(function (index, element) {
                if (index == 0) {
                    value += element.value;
                }
                else {
