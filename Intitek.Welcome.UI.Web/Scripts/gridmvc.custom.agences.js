...
function AgencesFilterWidget() {
    ...
    */
    this.onRender = function (container, lang, typeName, values, cb, data) {
        ...
        this.loadAgences(); //load customer's list from the server
    ...
    this.renderWidget = function () {
        var html = "";
        var template = Handlebars.compile($("#renderwidget-template-agences").html());
        html = template({ lang_applyFilterButtonText: this.lang.applyFilterButtonText });
        this.container.append(html);
    };
    /***
    * Method loads all Agences from the server via Ajax:
    */
    this.loadAgences = function () {
        var $this = this;
        openLoading();
        $.post(this.url, function (data) {
            $this.fillAgences(data.Items);
            closeLoading();
        });
    };
    /***
    * Method fill agences select list by data
    */
    this.fillAgences = function (items) {
        var filters = this.value.filterValue.split('|');
        var customerList = this.container.find(".agenceslist");
